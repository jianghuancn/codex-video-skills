...
- 每格只表达一个主要动作或视觉变化。
- 用景别拆分信息，不要把环境、动作、表情和关键细节全部塞进同一个镜头。需要电影感时优先按“远景/全景交代环境 -> 中景表现动作 -> 近景传递表情/眼神 -> 特写放大关键细节”的顺序组织镜头。
...
- 光线要服务情绪，不要默认全亮平光。根据场景选择冷色顶光、逆光轮廓光、底光、丁达尔光、柔和窗光、霓虹边缘光或月光，并写明阴影和氛围效果。
...
## Shot-Size And Lighting Storyboard Add-On

Use this block when a user's idea feels like a single flat prompt, or when they ask for more 电影感, 景别, 光线, 光影, 氛围, or visual storytelling.

```text
景别与光线要求：
- 用景别拆分信息并引导观看顺序：先用远景/全景交代[地点/整体环境]与[氛围]，再用中景表现[人物动作/行为关系]，接着用近景捕捉[表情/眼神变化]，最后用特写放大[关键细节/物品/手部/眼神]。
- 每个镜头只承担一个主要叙事功能：环境、动作、情绪或细节，不要把所有信息塞进同一画面。
- 光线服务情绪目标：[紧张/浪漫/恐怖/神圣/温暖/孤独]。
- 可选光线：冷色顶光用于神秘、紧张、危险、压迫；逆光/轮廓光用于浪漫、梦幻、温暖和发丝轮廓；底光用于诡异、不安、恐怖和反常；丁达尔光束用于教堂、森林、审判、图书馆等神圣或史诗空间。
